...
| Read-only files | **Verified.** Every file opens read-only; `M-x allow-editing` is the one deliberate way to edit (no shortcut can do it by accident) |
...
| Test suite | **302 tests, all pass in about 5 seconds** (`./build.sh test`); checked to catch six kinds of deliberate breakage |
