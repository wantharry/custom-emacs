...
run_one() {   # $1 = test file
  ...
  case "$harness" in
    ...
    config|noelpa)
      ...
      cp "$ROOT/config/early-init.el" "$ROOT/config/init.el" "$ROOT/config/fastfind.el" "$ROOT/config/startpage.el" "$dir/"
...
if [ "$GUI" = 1 ]; then
  ...
  if [ -z "${DISPLAY:-}${WAYLAND_DISPLAY:-}" ]; then
    ...
    cp "$ROOT/config/early-init.el" "$ROOT/config/init.el" "$ROOT/config/fastfind.el" "$ROOT/config/startpage.el" "$gdir/"
